...
export async function PATCH(
	...
	try {
		...
		const { id: researchId, requestID: requestId } = params;
		const { status } = await request.json();

		console.log('Updating research request:', {
			researchId,
			requestId,
			status,
			userId: user.id,
		...
		if (researchData?.createdById !== user.id) {
			console.log('Forbidden: User not creator', {
				createdById: researchData?.createdById,
				userId: user.id,
			});
			return NextResponse.json(
				{ error: 'Forbidden: Only research creator can manage requests' },
				{ status: 403 }
			);
		}
